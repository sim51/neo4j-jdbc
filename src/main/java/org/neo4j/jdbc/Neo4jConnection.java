...
public class Neo4jConnection extends AbstractConnection {
    ...
    private QueryExecutor createExecutor(String connectionUrl, String user, String password, Properties properties) throws SQLException {
        if (connectionUrl.contains("://")) {
            String remoteUrl = "http"+connectionUrl;
            final Resources.DiscoveryClientResource discovery = getDiscoveryResource(remoteUrl, user, password);
            if (!properties.containsKey(Driver.LEGACY) &&  discovery.getTransactionPath()!=null) return new TransactionalQueryExecutor(remoteUrl, user, password);
            if (discovery.getCypherPath()!=null) return new RestQueryExecutor(remoteUrl, user, password);
    ...
    private Resources.DiscoveryClientResource getDiscoveryResource(String connectionUrl, String user, String password) throws SQLException {
        try {
    ...
    public void commit() throws SQLException {
        checkClosed("commit");
        if (autoCommit) throw new SQLException("Commit called on auto-committed connection");
        try {
    ...
    public void rollback() throws SQLException {
        ...
        if (getAutoCommit()) throw new SQLException("Rollback called on auto-committed connection");
        try {
    ...
    private void checkClosed(String method) throws SQLException {
        if (isClosed())  throw new SQLException(method+" called on closed connection.");
    }
